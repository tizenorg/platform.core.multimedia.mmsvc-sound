...
static gpointer _tone_thread_func(gpointer userdata)
{
	...
	while (1) {
		...
		for (CurWaveIndex = 0; CurWaveIndex < numWave+1; CurWaveIndex++) {
			...
			if (prePlayingTime == h->duration || h->stop == 1) {
...
static int _prepare_sndfile(muse_sound_handle_s *h)
{
...
static void _unprepare_sndfile(muse_sound_handle_s *h)
{
	if (h->sf) {
		sf_close(h->sf);
		h->sf = NULL;
...
static int _prepare_tone(muse_sound_handle_s *h)
{
	/* FIXED SETTING */
...
static void _unprepare_tone(muse_sound_handle_s *h)
{
	return;
}
...
{
	if (h->mode == MODE_WAV) {
		if (_prepare_sndfile(h) == -1)
			return -1;
	} else {
		if (_prepare_tone(h) == -1)
			return -1;
	}

	_pa_connect_start_play(h);
...
int pulse_sound_stop(muse_sound_handle_s *h)
{
	...
	if (_pa_stop_disconnect(h) == -1)
		return -1;

	if (h->mode == MODE_WAV) {
		_unprepare_sndfile(h);
	} else {
		_unprepare_tone(h);
	}
